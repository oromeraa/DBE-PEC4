...
mensaje de error: me dice que no encuentra database y que pruebe con --database=mysql:8.0. Así hago:

```bash
ddev config --project-type=laravel --docroot=public --database=mysql:8.0
```
...
> **Nota:** He tenido que mover los que rehacerlo pero en un subdirectorio (laravel-website) ya que al tener archivos en el directorio principal, DDEV no me dejaba crear el proyecto.

```bash
ddev start
```
Me daba error de tunel y era porque tenía una vpn activada. La solucioné desactivando la vpn.

TODO: Además, compararemos el proceso de instalación de Laravel con el de Drupal en la PEC2.
